...
export const Sidebar: React.FC<SidebarProps> = ({ view, onViewChange }) => {
  ...
  const menuItems = [
    { icon: ShieldCheck, label: 'Güvenlik Özeti', id: 'summary' as ViewType },
    { icon: Search, label: 'Yeni Analiz', id: 'analysis' as ViewType },
    { icon: BarChart3, label: 'İstatistikler (OSINT)', id: 'osint' as ViewType },
  ];
  ...
  return (
    <div className={cn("flex flex-col transition-all duration-300 shrink-0 h-full", styles.container)}>
 ...
 className="w-full py-3 rounded-xl flex items-center justify-center gap-2 transition-all bg-blue-600 text-white font-bold shadow-md shadow-blue-200 active:scale-95"
        >
      ...
      <nav className="px-2 space-y-0.5 mb-2 flex-1">
        {menuItems.map((item) => (
          <button
            ...
            className={cn(
              "w-full flex items-center gap-2.5 p-2 rounded-lg transition-all group text-left",
              styles.item,
              view === item.id && styles.active // Aktif sayfa takibi eklendi
            )}
          >
      ...
      <div className="px-2 py-4 border-t border-gray-100 space-y-1">
        ...
        <button className={cn("w-full flex items-center gap-2.5 p-2 rounded-lg transition-all", styles.item)}>
          ...
          <span className={styles.label}>Yardım</span>
        </button>
        <button className={cn("w-full flex items-center gap-2.5 p-2 rounded-lg transition-all", styles.item)}>
