...
import { ViewType, Message, ChatSession } from '@/types';
...
export const AgentChat: React.FC<AgentChatProps> = ({ view, activeSession, onUpdateMessages }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Güvenlik nedeniyle API çağrısı backend'e taşınana kadar mock response simüle ediliyor
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      const mockResponses = [
        "Analiz tamamlandı. Bu görselin kaynağı doğrulanmış ve manipülasyon izine rastlanmamıştır. (Güvenilirlik: %98)",
        "⚠️ Dikkat: Blockchain ağında şüpheli bir işlem tespit edildi. Cüzdan adresi daha önce dolandırıcılık raporlarında yer almış.",
        "Sistem taraması sonucu: Belirtilen URL'deki içeriklerin phishing (oltalama) saldırılarında kullanıldığı raporlanmıştır."
      ];
      const responseText = mockResponses[Math.floor(Math.random() * mockResponses.length)] + "\n\n*(Not: Bu mesaj güvenlik amacıyla şimdilik simüle edilmiştir.)*";
